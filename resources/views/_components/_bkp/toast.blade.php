<div>
        ...
        <div class="fixed right-0 top-0 m-5 w-1/2 xl:w-1/5 lg:w-1/4 md:w-2/5 sm:w-1/2" >
            <template x-for="notice of notices" :key="notice.id">
                <div
                    ...
                    class="py-2 px-3 shadow-xl mb-2 border-r-4 grid grid-cols-4"
                    :class="{
    ...
    <button x-data="{}"
            ...
            class="m-4 bg-green-500 text-lg font-bold p-6 py-2 text-white shadow rounded">
        Success
    </button>
    <button x-data="{}"
            @click="$dispatch('notice', {type: 'info', text: 'آلرت اینفو'})"
            class="m-4 bg-blue-500 text-lg font-bold p-6 py-2 text-white shadow rounded">
        Info
    </button>
    <button x-data="{}"
            @click="$dispatch('notice', {type: 'warning', text: 'آلرت وارنینگ'})"
            class="m-4 bg-orange-500 text-lg font-bold p-6 py-2 text-white shadow rounded">
        Warning
    </button>
    <button x-data="{}"
            x-on:click="$dispatch('notice', {type: 'danger', text: 'آلرت دنجر'})"
            class="m-4 bg-red-500 text-lg font-bold p-6 py-2 text-white shadow rounded">
        Danger
    </button>
        ...
        function noticesHandler() {
            return {
                ...
                fire(id) {
                    ...
                    const timeShown = 500 * this.visible.length
                    setTimeout(() => {
                        this.remove(id)
                    }, timeShown)
                },
